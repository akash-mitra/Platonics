...
class Media extends Model
{
	...
	{
		if ($this->storage === 's3') return $this->CDNWrap($this->uri);
	...
	public static function store ($uploadedFile, $subDirectoryPath = '', $allowedExtensions = ['jpeg', 'png', 'jpg'], $maxSize = 1)
	{
		...
		$storageConfig = Configuration::retrieveObjectByKey('storage', StorageConfig::class);
	...
	{
		 self::clientError(sprintf ('File [%s] upload error: %s', 
	...
	{
		 self::clientError(sprintf ('File [%s] size is too large (%sMB). Must be less than %sMB.', 
					 $uploadedFile->getClientOriginalName(), 
	...
	{
		 self::clientError(sprintf ('Type of file [%s] is determined as (%s). This extension type is not accepted.', 
					 $uploadedFile->getClientOriginalName(), 
